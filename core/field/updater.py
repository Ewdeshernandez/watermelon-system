"""
core/field/updater.py — Auto-actualizador del módulo NATIVO (Watermelon Rotordynamics)
==============================================================================

Cuando el PC de campo se conecta a internet, la app consulta los Releases de
GitHub (tags `field-v*`), compara con su versión local y, si hay una más nueva,
avisa y permite actualizar de una: descarga el instalador `WatermelonRotordynamics-Setup.exe`
...
REPO = os.environ.get("WM_FIELD_REPO", "Ewdeshernandez/watermelon-system")
_UA = {"User-Agent": "WatermelonRotordynamics-Updater", "Accept": "application/vnd.github+json"}
...
def check_for_update(current_version: str, timeout: float = 6.0) -> Optional[Dict[str, Any]]:
    """Devuelve info del release más nuevo (`field-v*`) si supera a current_version,
    ...
    try:
        ...
        # así que con 30 el field-v* más nuevo podía caer fuera de la primera página.
    ...
    for rel in rels or []:
        ...
        if not tag.startswith("field-v") or rel.get("draft"):
            continue
        v = _parse_ver(tag)
...
def diagnose(current_version: str, timeout: float = 6.0):
    ...
    tags = [x.get("tag_name") for x in (rels or []) if str(x.get("tag_name", "")).startswith("field-v")]
...
def download_file(url: str, dest: Optional[str] = None, timeout: float = 60.0,
    ...
    for attempt in range(max(1, retries)):
        ...
        headers = {"User-Agent": "WatermelonRotordynamics-Updater"}
        if got > 0:
