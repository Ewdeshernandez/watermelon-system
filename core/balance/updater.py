"""
core/balance/updater.py — Auto-actualizador del módulo NATIVO (Watermelon Balance)
==============================================================================

Cuando el PC de campo se conecta a internet, la app consulta los Releases de
GitHub (tags `balance-v*`), compara con su versión local y, si hay una más nueva,
avisa y permite actualizar de una: descarga el instalador `WatermelonBalance-Setup.exe`
...
_UA = {"User-Agent": "WatermelonBalancing-Updater", "Accept": "application/vnd.github+json"}
...
def check_for_update(current_version: str, timeout: float = 6.0) -> Optional[Dict[str, Any]]:
    """Devuelve info del release más nuevo (`balance-v*`) si supera a current_version,
    ...
    try:
        ...
        # así que con 30 el balance-v* más nuevo podía caer fuera de la primera página.
    ...
    for rel in rels or []:
        ...
        if not tag.startswith("balance-v") or rel.get("draft"):
            continue
        v = _parse_ver(tag)
...
def diagnose(current_version: str, timeout: float = 6.0):
    ...
    tags = [x.get("tag_name") for x in (rels or []) if str(x.get("tag_name", "")).startswith("balance-v")]
...
def download_file(url: str, dest: Optional[str] = None, timeout: float = 60.0,
    ...
    for attempt in range(max(1, retries)):
        ...
        headers = {"User-Agent": "WatermelonBalancing-Updater"}
        if got > 0:
