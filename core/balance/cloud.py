"""
core/balance/cloud.py — Guardado/subida a la NUBE de corridas balancees
===========================================================================

Espejo de `core.modal.modal_cloud` para el módulo Balance: sube la CORRIDA
...
Requiere en Supabase la tabla `balance_runs` (id text PK, name text,
metadata jsonb, updated_at text, account/client/tag/hostname text) y el bucket
de Storage `balance-raw`. Reusa el cliente embebido del recorder (mismas
...
_RUNS_TABLE = "balance_runs"
_RAW_BUCKET = "balance-raw"
...
def save_run(name: str, payload: Dict[str, Any], run_id: str = "", ts: str = "",
    ...
    """Sube una corrida balance (metadata) a la tabla `balance_runs`.
    ...
    try:
        ...
        row = {"id": run_id, "name": name or "Balance run", "metadata": payload, "updated_at": ts,
               "account": account or "", "client": client or "", "tag": tag or "", "hostname": hostname or "",
               "ip": ip or "", "geo": geo or "", "module": "Balance"}
        try:
...
def list_runs() -> List[Dict[str, Any]]:
    """Lista las corridas balancees subidas por el campo (para elegir en la web)."""
    c = _client()
    if c is None:
        return []
    try:
...
def load_run(run_id: str) -> Optional[Dict[str, Any]]:
    """Descarga el payload (metadata) de una corrida balance por su id."""
    c = _client()
    if c is None:
        return None
    try:
