    ...
    def read_block(self) -> np.ndarray:
        ...
        for ci, ch in enumerate(cfg.channels):
            ...
            if kind == "prox":
                ...
                ecc = np.clip(1.0 - rpm / (3.5 * rc), 0.03, 1.0)     # 1 en reposo → ~0 alta vel
                att = (np.pi / 2.0) * (1.0 - ecc)                    # attitude 0°→90° (locus CURVO)
                Cmil = 8.0                                            # clearance (mil)
                jx = Cmil * ecc * np.sin(att)                        # muñón (mil) — X derecha
                jy = -Cmil * ecc * np.cos(att)                       # abajo en reposo
