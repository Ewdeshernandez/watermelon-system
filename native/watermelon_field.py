...
def main() -> int:
    ...
    pending = {"path": (args.machine_file or None)}
    ...
    def _build_window():
        ...
        def do_apply():
            """Aplica la config y reconstruye la medición EN CALIENTE (mismo proceso, sin
            relanzar el .exe ni disparar SmartScreen): valida, guarda a un temporal y pide
            al loop de main que rearme la ventana con la nueva máquina."""
            import tempfile, types
            m = read_form()
            path = os.path.join(tempfile.gettempdir(), "wm_apply_machine.json")
            m.save(path)
            try:                                    # validar que la config se puede construir
                ...
                QtWidgets.QMessageBox.critical(
                    ...
                    f"measured:\n\n{type(e).__name__}: {e}")
                return
            pending["path"] = path                  # el loop de main reconstruye en caliente
            app.quit()
    ...
    while True:
        ...
        if _old is not None:
            for _stopper in (getattr(_old, "_timer", None), getattr(_old, "_agent", None)):
                try:
                    _stopper.stop()
                except Exception:
                    pass
            try:
                ...
                pass
        app.exec()
        if not pending.get("path"):
            break
    try:
        cur["win"]._timer.stop(); cur["win"]._agent.stop()
    except Exception:
        pass
    return 0
