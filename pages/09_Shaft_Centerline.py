...
def render_scl_panel(
    ...
    *,
    ...
    with st.expander(f"Panel {panel_index + 1} · Geometry / Scale / Clearance", expanded=False):
        st.markdown("#### X / Y Scale")
        s1, s2, s3, s4 = st.columns(4)

        auto_scale_xy = s1.checkbox(
            "Auto X/Y",
            value=True,
    ...
    if normalize_to_origin:
        base_x = float(plot_df["x_gap"].iloc[0])
        base_y = float(plot_df["y_gap"].iloc[0])
        x_plot = (plot_df["x_gap"] - base_x).to_numpy(dtype=float)
        y_plot = (plot_df["y_gap"] - base_y).to_numpy(dtype=float)
    else:
        x_plot = plot_df["x_gap"].to_numpy(dtype=float)
        y_plot = plot_df["y_gap"].to_numpy(dtype=float)
    ...
    export_state_key = (
        f"scl::{item['id']}::{panel_index}::{smooth_window}::{show_info_box}::{show_rpm_labels}::{marker_stride}::"
        f"{normalize_to_origin}::{auto_scale_xy}::{manual_x_min}::{manual_x_max}::{manual_y_min}::{manual_y_max}::"
