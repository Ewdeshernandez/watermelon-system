...
Admin UI para gestionar el registry multi-tenant de Watermelon
(`data/clients.json`) sin tocar código.

Ciclo 20B — solo admin. Specialists y client no pueden entrar.
Prefix `_` la oculta de la nav automática de Streamlit; el wiring
para mostrarla en el sidebar manual se agrega en core/auth.py
NAV_ITEMS bajo el rol admin.

3 tabs:
  - Clientes     → CRUD sobre clients.json[clients]
  - Specialists  → CRUD sobre clients.json[specialists]
  - Admins       → CRUD sobre clients.json[admins] (cuidado, no
                   te puedes desautorizar a ti mismo)

Cada cambio:
  - Valida estructura mínima
  - Escribe atómicamente clients.json (tmp + rename)
  - Limpia cache lru_cache → el cambio toma efecto inmediato
  - Queda en git history cuando hagas commit
"""
...
def _phones_to_list(text: str) -> List[str]:
    """'573008888883, +57 310 677 6206' → ['573008888883', '573106776206']"""
    raw = re.split(r"[,\n;]+", text or "")
    out = []
    for p in raw:
...
# clients sin api_key visible — la mantenemos en memoria pero no la mostramos
# salvo cuando el admin pide regenerarla.
...
with tab_clients:
    ...
    st.caption(
        "Cada cliente solo verá los activos cuyos `report_meta.client / "
        ...
        with st.form("client_form"):
            ...
            with col_a:
                new_id = st.text_input(
                    "ID (slug, no editable si ya existe)",
                    value=current["id"],
            ...
            with col_b:
                new_assets = st.text_input(
                    "Asset tags informativos (separados por coma)",
                    value=", ".join(current["asset_tags"]),
                    placeholder="TES1, TES3, C-200-C",
                )
                new_phones = st.text_area(
                    "WhatsApp numbers (uno por línea o coma)",
                    value=_list_to_text(current["whatsapp_numbers"]),
                    height=70,
                    placeholder="573185551234, 573185559876",
                )
                new_owners = st.text_area(
                    "Owner emails (separados por coma)",
                    value=", ".join(current["owner_emails"]),
                    height=70,
                    placeholder="mantenimiento@cliente.com",
                )
            ...
            if save_clicked:
                ...
                if not target_id:
                    ...
                    new_entry = {
                        ...
                        "match_strings": [
                            s.strip().lower()
                            for s in new_match.split(",")
                            if s.strip()
                        ],
                        "asset_tags": [
                            s.strip()
                            for s in new_assets.split(",")
                            if s.strip()
                        ],
                        "whatsapp_numbers": _phones_to_list(new_phones),
                        "owner_emails": [
                            e.strip().lower()
                            for e in new_owners.split(",")
                            if e.strip()
                        ],
                        "api_key": "",
                    }
                    # Preservar api_key existente si lo había
                    if sel_id != "— nuevo —":
                        existing = next(
                            (c for c in clients_data if c["id"] == sel_id),
                            None,
                        )
                        # el dict de list_clients NO incluye api_key (privacidad).
                        # Cargamos del raw json directamente para no perderla.
                        from core.clients import _load_registry
                        raw = _load_registry()
                        for c_raw in raw.get("clients", []):
                            if c_raw.get("id") == sel_id:
                                ...
                                break
                        new_clients = [
                            new_entry if c["id"] == sel_id else
                            _enrich_with_apikey(c)
                            for c in clients_data
                        ]
                    else:
            ...
            if del_clicked and sel_id != "— nuevo —":
                new_clients = [
                    _enrich_with_apikey(c) for c in clients_data if c["id"] != sel_id
                ]
                try:
                    save_registry(_build_full_data(
                        admins=admins_data,
...
with tab_specialists:
    ...
    st.caption(
        "Tienen el mismo nivel de visibilidad que admin (ven TODOS los "
    ...
    sel_spec = st.selectbox(
        "Seleccionar specialist",
        options=["— nuevo —"] + spec_emails,
    ...
    with st.form("spec_form"):
        ...
        with col_del:
            del_spec = st.form_submit_button(
                "🗑️ Eliminar",
                use_container_width=True,
                disabled=(sel_spec == "— nuevo —"),
            )
        ...
        if save_spec:
            if not new_name.strip() or not new_email.strip():
                ...
                if sel_spec == "— nuevo —":
                    if any(s["email"] == new_entry["email"]
                           for s in specialists_data):
                        st.error(f"Ya existe specialist con email "
                                 f"'{new_entry['email']}'.")
                        st.stop()
                    new_specs = specialists_data + [new_entry]
                else:
                    new_specs = [
                        new_entry if s["email"] == sel_spec else s
                        for s in specialists_data
                    ]
                try:
                    save_registry(_build_full_data(
                        admins=admins_data,
                        specialists=new_specs,
                        clients=[_enrich_with_apikey(c) for c in clients_data],
                    ))
...
with tab_admins:
    st.subheader("Admins")
    st.warning(
        "⚠️ Cuidado: los admins gestionan TODO el sistema. "
    ...
    with st.form("admin_form", clear_on_submit=True):
        ...
        with col_b:
            new_phones = st.text_area(
                "WhatsApp numbers (separados por coma)",
                placeholder="573185551234",
                height=100,
            )
        ...
        if save_adm:
            if not new_name.strip() or not new_email.strip():
                st.error("Nombre y email son obligatorios.")
            elif any(a["email"].lower() == new_email.strip().lower()
                     for a in admins_data):
