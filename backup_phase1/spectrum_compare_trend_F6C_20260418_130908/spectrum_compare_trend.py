...
def build_trend_action_priority(
    severity_level: Optional[str],
    trend_label: Optional[str],
) -> Dict[str, str]:

    severity = str(severity_level or "").lower()
    trend = str(trend_label or "").lower()

    action = "Monitorear"
    color = "#16a34a"
    text = "No se requiere acción inmediata. Continuar monitoreo."

    if severity == "alta" and trend == "worsening":
        action = "Atención prioritaria"
        color = "#dc2626"
        text = "Se recomienda intervención inmediata para evitar falla o daño progresivo."

    elif severity == "moderada" and trend in ["worsening", "volatile"]:
        action = "Programar intervención"
        color = "#f59e0b"
        text = "Se recomienda planificar intervención en próxima ventana de mantenimiento."

    elif severity == "leve" and trend == "improving":
        action = "Monitorear"
        color = "#16a34a"
        text = "La condición mejora. Mantener seguimiento como control."

    elif severity == "leve" and trend == "stable":
        action = "Monitorear"
        color = "#16a34a"
        text = "La condición es estable. Continuar monitoreo periódico."
