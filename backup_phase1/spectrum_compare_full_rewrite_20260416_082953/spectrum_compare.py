...
def build_compare_assessment(
    ...
    *,
    ...
    secondary_fault = "—"
    executive_summary = (
        "No se observa un cambio dominante claramente atribuible a un mecanismo de falla específico."
    )
    technical_basis = (
    ...
    recommendation = (
        "Mantener esta comparación como referencia base y correlacionar con Orbit, Bode, Trends y condición operativa."
    )
    ...
    if (
        ...
        executive_summary = (
            "B presenta crecimiento dominante en 1X respecto a A."
        )
        technical_basis = (
        ...
        recommendation = (
            "Verificar balanceo, condición del rotor y estabilidad de fase; correlacionar con módulos Polar y Bode."
        )
    ...
    if (
        ...
        executive_summary = (
            "B incrementa simultáneamente las componentes 1X y 2X frente a A."
        )
        technical_basis = (
        ...
        recommendation = (
            "Revisar alineación, condición del acople, rigidez de soportes y correlacionar con mediciones axiales y análisis de fase."
        )
    ...
    if (
        ...
        executive_summary = (
            "B incrementa 2X y 3X respecto a A."
        )
        technical_basis = (
        ...
        recommendation = (
            "Revisar alineación, acople, fijaciones mecánicas y confirmar con forma de onda y fase."
        )
    ...
    if high_harm_delta_pct is not None and high_harm_delta_pct >= 25:
        ...
        executive_summary = (
            "B muestra crecimiento importante en armónicos altos respecto a A."
        )
        technical_basis = (
        ...
        recommendation = (
            "Inspeccionar bases, pernos, soportes, pedestal, holguras y comportamiento de impactos en forma de onda."
        )
    ...
    if (
        ...
        executive_summary = (
            "La energía global crece más que el pico dominante."
        )
        technical_basis = (
        ...
        recommendation = (
            "Revisar condición de proceso, cavitación, turbulencia, roce o excitaciones no estacionarias."
        )
    ...
    if peak_delta_pct is not None and abs(peak_delta_pct) <= 8 and (overall_delta_pct is None or abs(overall_delta_pct) <= 8):
        ...
        recommendation = (
            "Mantener monitoreo y usar esta comparación como línea base."
        )
...
def build_compare_validation_table(
    ...
    rows = [
        ...
        },
        {
            ...
            "Δ / Status": format_number(safe_pct_change(summary_b.get("rpm"), summary_a.get("rpm")), 1) + "%" if safe_pct_change(summary_b.get("rpm"), summary_a.get("rpm")) is not None else "—",
        },
        {
            ...
            "Δ / Status": format_number(safe_pct_change(summary_b.get("sample_rate_hz"), summary_a.get("sample_rate_hz")), 1) + "%" if safe_pct_change(summary_b.get("sample_rate_hz"), summary_a.get("sample_rate_hz")) is not None else "—",
        },
        {
            "Validation Parameter": "Duration (s)",
            "A": format_number(summary_a.get("duration_s"), 3),
            "B": format_number(summary_b.get("duration_s"), 3),
            "Δ / Status": format_number(safe_pct_change(summary_b.get("duration_s"), summary_a.get("duration_s")), 1) + "%" if safe_pct_change(summary_b.get("duration_s"), summary_a.get("duration_s")) is not None else "—",
        },
    ]
    return pd.DataFrame(rows)
