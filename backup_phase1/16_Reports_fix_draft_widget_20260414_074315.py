...
with d1:
    draft_name = st.text_input(
        "Nombre del borrador",
        key="report_draft_name",
        value=st.session_state.get("report_draft_name", "") or (meta.get("consecutive") or meta.get("asset") or "reporte_actual"),
    )
with d2:
    st.write("")
    st.write("")
    if st.button("Guardar borrador", use_container_width=True):
        ...
        st.session_state["report_draft_name"] = saved_name
...
with d6:
    ...
    if st.button("Cargar borrador", use_container_width=True, disabled=not drafts or selected_draft == "—"):
        ...
        st.session_state["report_pdf_bytes"] = None
        st.session_state["report_pdf_error"] = None
        st.session_state["report_draft_name"] = selected_draft
        save_report_state(items=st.session_state["report_items"], meta=st.session_state["report_meta"])
...
with d7:
    ...
    if st.button("Eliminar borrador", use_container_width=True, disabled=not drafts or selected_draft == "—"):
        ...
        if st.session_state.get("report_draft_name") == selected_draft:
            st.session_state["report_draft_name"] = ""
